					...
					<tbody className="divide-y divide-gray-100">
						{localData?.map(
							({ version, status, v_date }, index) => {
								...
								const v1_date = prevVersion
									? prevVersion.v_date
									: "N/A"; // Get v_date of version (N-1)
                          ...
                          `}
											...
											{activeVersion !== version && (
												<div
													className="absolute right-full top-1/2 -translate-y-1/2 mr-2 w-48 rounded-lg bg-white px-3 py-2 text-xs text-gray-900 border border-gray-200 shadow-md z-10
																	opacity-0 transition-opacity duration-200 group-hover:opacity-100"
												>
													Cannot change status unless the
													version is accessed first.
